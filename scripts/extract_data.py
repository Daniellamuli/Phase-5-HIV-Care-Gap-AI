...
Real file structures (confirmed from notebook 01):
  ART  (47x11) : Period + County, "Baringo County" (strip suffix)
  HTS  (47x7)  : Period + County, "Baringo County" (strip suffix)
  HTS+ (47x?)  : Period + County, same format as HTS (HIV+ counts)
  VLT  (47x7)  : County ONLY, no Period, "Baringo" (clean)
  IIT  (9x13)  : Region ONLY, no Period, 9 MOH regions
...
from constants import (
    ADULT_ART_FILE, HTS_FILE, VLT_FILE, IIT_FILE,
    DHS_REDUCED, HTS_POSITIVE_FILE,
...
def load_with_period(filepath, period_col, label):
    """ART, HTS, HTS+ — have both Period and County columns."""
...
def extract_all():
    """
    Load all 6 raw source files, print diagnostics,
    validate all county/region/code mappings.
    ...
    hts_pos = load_with_period(HTS_POSITIVE_FILE, HTS_PERIOD_COL, "HTS+")
    ...
    print(f"  {'HTS_Positive.xlsx':<25} {hts_pos.shape[0]:>5} {hts_pos.shape[1]:>5}  {'Yes':>8}  'Baringo County' → strip suffix")
    print(f"  {'VLT.xlsx':<25} {vlt.shape[0]:>5} {vlt.shape[1]:>5}  {'No':>8}  'Baringo' → clean, no suffix")
    print(f"  {'IIT.xlsx':<25} {iit.shape[0]:>5} {iit.shape[1]:>5}  {'No':>8}  9 regions → expand to 47 counties")
    print(f"  {'individual_features.csv':<25} {dhs.shape[0]:>5} {dhs.shape[1]:>5}  {'N/A':>8}  integers 1-47 → DHS_COUNTY_MAP")
    ...
    errors += validate_county_map(hts_pos[HTS_COUNTY_COL].dropna().unique(), "HTS+", HTS_COUNTY_SUFFIX)
    ...
    # Validate REGION_TO_COUNTIES covers all 47
    ...
    return {"art": art, "hts": hts, "hts_pos": hts_pos,
            "vlt": vlt, "iit": iit, "dhs": dhs}
